...
void do_async_setting(void *param)
{
	...
	pr_info("do_async_setting : %d-%s-%s + \n", setting_work->type,
		setting_work->setting_workqueue->name, setting_work->name);
	setting_work->write_setting(setting_work->ctx);
	pr_info("do_async_setting : %d-%s-%s - \n", setting_work->type,
		setting_work->setting_workqueue->name, setting_work->name);
			...
			struct subdrv_ctx *ctx)
{
	...
	if (!name || !ctx || !write_setting || !setting_workqueue) {
...
struct setting_workqueue* create_setting_workqueue(const char *name)
{
	...
	strcpy(setting_workqueue->name, name);
